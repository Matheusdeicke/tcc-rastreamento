...
<div class="max-w-3xl mx-auto p-4 sm:p-6">
  <h3 class="text-2xl font-bold text-brand-800">Editar instância — {{ $kit->nome }}</h3>

  <form method="POST" action="{{ route('instances.update',$instance) }}"
    ...
    <div>
      ...
      <input name="etiqueta" required value="{{ old('etiqueta',$instance->etiqueta) }}"
    ...
    <div>
      ...
      <select name="status" required
        ...
        @foreach($status as $st)
          <option value="{{ $st }}" @selected(old('status',$instance->status)===$st)>{{ $st }}</option>
        @endforeach
      </select>
    </div>
    ...
    <div>
      ...
      <input type="datetime-local" name="data_validade"
             value="{{ old('data_validade', optional($instance->data_validade)->format('Y-m-d\TH:i')) }}"
             class="w-full rounded-lg border-gray-300 focus:border-brand-accent focus:ring-brand-accent">
    </div>
